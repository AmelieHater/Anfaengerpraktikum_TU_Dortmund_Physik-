...
n_p_mean = ufloat(np.mean(n_p[n_p < 6 ]), np.std(n_p[n_p < 6]))
...
ax1.plot(x * 180/np.pi, fresnel_senkrecht(np.mean(n_s[n_s < 4]), x), '-', color='indigo', label='Theoriekurve senkrecht')
...
ax1.plot(x_1, fresnel_parallel(np.mean(n_p[n_p < 6 ]) , x_1*np.pi/180), '-', color='orangered', label='Theoriekurve parallel')
ax1.plot(x_2, -1*fresnel_parallel(np.mean(n_p[n_p < 6 ]), x_2*np.pi/180), '-', color='orangered')
...
print(f'Diskussion------------')
n_rel_s = rel_Abweichung(np.mean(n_s[n_s < 4]), n_theo)
n_rel_p_reduziert = rel_Abweichung(np.mean(n_p[n_p < 6]), n_theo)
