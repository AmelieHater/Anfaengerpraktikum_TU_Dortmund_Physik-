...
L_x_19_d = np.mean(L_x_19_d) # in H
...
L_x_16_d = np.mean(L_x_16_d) # in H
print (f"L_x_19_d: {L_x_16_d * 10**3} in mH")
...
def Theoriekurve(x):
    return (1/9)* ((x**2 -1)**2) / ((1 - x**2)**2 + 9 * x**2)
...
ax1.set_xscale("log")
ax1.set_xlim([0.1,14])
ax1.set_xlabel(r"$\Omega = \frac{f}{f_0}$")
...
U_br_2_f = 15* 10**(-3) / (2* np.sqrt(2))
U_2_f = U_br_2_f/f_2_f
print (f"U_2_f: {U_br_2_f} in V")
print (f"f_2_f: {f_2_f}")
k = U_2_f
print(f"k: {k}")