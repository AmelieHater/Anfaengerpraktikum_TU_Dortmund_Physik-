...
verwendet. 
Der Fehler von $\rho_{\text{kl}}$ wird durch die Gaußsche Fehlerfortpflanzung berechnet. Eingesetz ergibt sich 
$$\Delta \rho_{\text{kl}} = \sqrt{\left(-\frac{m_{\text{kl}}}{V_{\text{K}}^2}\cdot \Delta V_{\text{K}}\right)^2}$$ 
Es folgt
...
\end{table}
\noindent
Anhand dieser Messdaten ergeben sich die folgenden gemittelten Zeiten sowie deren Abweichung zum Mittelwert.
\begin{align*}
  t_{\text{kl,r}} &= \left(12.14\pm0.11\right)  \unit{\second}\\
  t_{\text{kl,h}} &= \left(12.20\pm0.13\right)  \unit{\second}\,.
\end{align*}
Diese Werte werden zur Berechnung der Viskosität benötigt.
...
\begin{table}[H]
  ...
  \end{tblr}
\end{table}
Aus den Messdaten werden die folgenden gemittelten Zeiten mit deren Standardabweichung bestimmt:
%
\begin{align*}
...
Zur Berechnung der Reynoldszahl wird die charakteristische Länge des Experiments benötigt. 
Diese entspricht annähernd dem Durchmesser der großen Kugel $d_{\text{gr}}$.
...
\begin{align}
  \begin{split}
   \eta (T) &= A \cdot e^{\frac{B}{T}}\\
    \Rightarrow \ln\left(\eta\left(T\right)\right) &= \frac{B}{T} + \ln\left(A\right)
    \label{eqn:lnEta}
  \end{split}
\end{align}
...
\begin{table}[H]
  ...
  \end{tblr}
\end{table}
Im Abschnitt \ref{sec:Viskosität von Wasser} wird bereits festgestellt, dass die dynamischen Viskositäten 
...
betrachtet. Demnach ergeben sich für die jeweiligen Temperaturen die folgenden Fallzeiten:
\begin{table}[H]
  \centering 
...
deren Unsicherheiten in Abbildung (\ref{fig:plot}) graphisch darstellen. 
\begin{figure}[H]
  \centering
...
\begin{align*}
  \ln\left(A\right) &= -5.567 \pm 0.097 \Longleftrightarrow A = (0.0038\pm0.0004)\,\unit[inter-unit-product=\cdot]{\milli\pascal\second}\\
  B &= (1680\pm30)\, \unit{\kelvin}
\end{align*}
