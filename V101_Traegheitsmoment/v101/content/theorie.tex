...
\label{sec:Theorie}
Rotationsbewegungen lassen sich mithilfe des Drehmoments $M$, des Trägheitsmoment $I$ und 
...
geometrisch einfache Formen aufgeteilt, deren Trägheitsmomente zu einem Gesamtträgheitsmoment 
...
mit einem Abstand $a$ zu einer Schwerpunktachse verläuft, dann lässt sich mithilfe des Satz von Steiner
das Trägheitsmoment durch
\begin{equation}
...
Das wirkende Drehmoment wird mithilfe der Winkelrichtgröße $D$ durch die Gleichung
\begin{equation}
   M = D \cdot \varphi\,
...
durch
\begin{align}
    I_{\text{Z}} &= \frac{m \cdot r^2}{2} \label{eqn:TragheitZylinder} \,\, \text{und}\\
    I_{\text{Zh}} &= m \left(\frac{r^{2}}{4} + \frac{h^{2}}{12} \right) \label{eqn:TrägheitZylinderQuer}\,.
\end{align}
Hier beschreibt $I_{\text{Z}}$ das Trägheitsmoment eines Zylinders, dessen Drehachse senkrecht durch die Grundfläche verläuft.  
$I_{\text{Zh}}$ ist das Trägheitsmoment eines Zylinders, dessen Drehachse senkrecht zur Mantelfläche steht. 
%
...
einmal mit einem Winkel $\varphi = 90°$ und einmal mit einem Winkel $\varphi = 45°$ auf eine Stange. Das Drehmoment wird mithilfe der Gleichung 
(\ref{eqn:DrehmomentSkalar}) berechnet und sind in der Tabelle (\ref{tab:Vorbereitungsaufgabe}) aufgeführt. 
\begin{table}
    \centering
    \caption{Berechnetes Drehmoment in Abhängigkeit von zwei verschiedenen Winkeln und verschiedenen Abständen}
